...
更推荐为每个 agent 或每次测试任务创建一套独立的临时环境。
...
这样 Docker 会自动创建独立的 container、network、volume，不同 agent 之间不会互相影响。
...
**注意**：`ports: - "3000"` 表示让 Docker 随机分配 host port，避免多个 agent 同时运行时端口冲突。不要写成 `"3000:3000"`，否则多个环境同时运行时会冲突。

---

## 6. Isolated E2E 脚本示例

创建 `scripts/e2e-run-isolated.sh`：
...
创建 `scripts/wait-for-services.sh`：
...
职责：修改代码、实现 feature、修复 bug、运行 targeted tests、必要时调用 `e2e-isolated`。

### Test Agent

职责：创建或更新 Playwright E2E tests、启动 isolated E2E environment、验证用户流程、收集失败截图和 trace。

### Security Agent

职责：检查登录、鉴权、CORS、CSRF、JWT、敏感日志，确认 E2E 不使用 production secrets，确认测试账号和测试数据安全。

### Review Agent

职责：检查最终 diff、检查测试是否覆盖关键路径、确认环境被清理、总结风险和未覆盖场景。
...
推荐方式：WireMock、MockServer、sandbox API、test tenant。
...
优点：更接近真实云环境、适合 PR preview environment、适合多服务系统、适合公司级 CI/CD。

缺点：搭建复杂、需要 DevOps 支持、成本更高、清理机制必须可靠。

---

## 15. 推荐落地路线

**Phase 1：本地 Docker Compose** — 让 Claude Code / AI agent 能一键启动临时 E2E 环境。实现：`docker-compose.e2e.yml` + `scripts/e2e-run-isolated.sh` + Makefile + Playwright config + CLAUDE.md instructions。

**Phase 2：CI 集成** — 每次 PR 或 main build 跑 isolated E2E。Jenkins / GitHub Actions / GitLab CI，archive Playwright report，加 quality gate。

**Phase 3：Preview Environment** — 每个 PR / agent 拥有接近 production 的临时环境。Kubernetes namespace per PR，Helm deployment，temporary database，sandbox external services，automatic cleanup。

---

## 16. 总结

AI agent 做 E2E 测试时，**不要共用一个固定测试环境，而是为每个 agent 或每次任务创建独立的临时环境**。
...
这样可以实现：环境独立、测试稳定、可重复执行、不污染共享环境、不访问 production、适合 Claude Code / AI agent 自动测试。
