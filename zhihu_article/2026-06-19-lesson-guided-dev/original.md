# Lesson-Guided Development for Claude Code
...
> 以前的 lesson 要在本次编程过程中主动变成 coding guidance，提前约束 agent，防止它再次犯同样错误。
...
例如 commit/eval 后，把新 lesson 追加到：
...
你的目标不是简单引用 dev log，也不是让 `CLAUDE.md` 保存所有 lesson。
