...
直接跟 Claude Code 说「写一篇公众号文章，思路是…」或 `/juejin-publishing`，把草稿粘进来。skill 会问 1–2 个问题，然后产出文章目录：
...
~/.claude/skills/juejin-publishing/scripts/gen-cover-ai.sh "$DIR" "目标字词"

# 解释图（比例自由，不裁）
~/.claude/skills/juejin-publishing/scripts/gen-illustration.sh "$DIR"

# 发布：建/更新草稿到 mp.weixin.qq.com 后台
~/.claude/skills/juejin-publishing/scripts/upload-draft.sh "$DIR"
```
...
```bash
git clone https://github.com/jianshuo/juejin-publishing ~/.claude/skills/juejin-publishing
```

之后在 Claude Code 里说「写一篇公众号」「润色」「准备发布」或 `/juejin-publishing` 就会触发。
